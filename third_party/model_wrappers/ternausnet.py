#source: https://github.com/ternaus/robot-surgery-segmentation
...
class TernausNet11(nn.Module):
    """ U-net with a VGG11 backbone, the backbone can be pre-trained or not.
        Since the encoder can be pre_trained but the decoder cannot, it is important to take that into account during training
        and experiment with freezing the encoder for a certain number of epochs (only when pretrained=True).
    """
    def __init__(self, num_classes=1, num_filters=32, pretrained=False):
        """
        :param num_classes: 
        :param num_filters: for the decoder
        :param pretrained: 
    ...
    def forward(self, x):
        ...
        # VGG only takes RGB as input, so if the image has only one channel we stack to make it 3.
        if x.shape[1]==1:
...
class TernausNet16(nn.Module):
    """ U-net with a VGG16 backbone, the backbone can be pre-trained or not.
        Since the encoder can be pre_trained but the decoder cannot, it is important to take that into account during training
        and experiment with freezing the encoder  for a certain number of epochs (only when pretrained=True).
    """
    def __init__(self, num_classes=1, num_filters=32, pretrained=False):
...
class LinkNet34(nn.Module):
    """ U-net with a ResNet34 backbone, the backbone can be pre-trained or not.
        Since the encoder can be pre_trained but the decoder cannot, it is important to take that into account during training
        and experiment with freezing the encoder completely for a certain number of epochs (only when pretrained=True).
    """
